...
# ANN test errors
ann_errors = np.array([0.5, 0.38, 0.25, 0.38, 0.5, 0.25, 0.12, 0.12, 0.5, 0.0])
...
def find_p_and_ci(error_array1, error_array2):
    ...
    for i in range(10):
        if i != 9:
            r[i] = abs(1/8 * (error_array1[i] - error_array2[i]))
        else:
            r[i] = abs(1/7 * (error_array2[i] - error_array2[i]))
...
find_p_and_ci(baseline_errors,ann_errors)
