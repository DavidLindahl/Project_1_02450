...
for outer_fold, (train_index, test_index) in enumerate(outer_cv.split(st_X, y)):
    print(f"running outer fold {outer_fold+1}/{outer_k}...")
    # Split data
    X_train_outer, X_test_outer = st_X[train_index], st_X[test_index]
    ...
    table += f"{outer_fold},{best_nodes_number},{outer_mse_ann[outer_fold]},{best_alpha},{outer_mse_lr[outer_fold]},{baseline}\n"
